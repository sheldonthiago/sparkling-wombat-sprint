...
Then modify InventoryPage to import and use it.

We need to add import statements.

Let's write ExportManager.tsx:

"use client";

import { Button, DropdownMenu, MenuItem } from "@/components/ui/button";
import { IconNameButton } from "@/components/ui/icon-name-button";
import { useSupabaseInventory } from "@/hooks/use-supabase-inventory";
import { ExportButton } from "@/components/ui/export-button"; // maybe not exist. Use Button with onClick.

But we can just use Button with icons.

We'll create functions that receive data and trigger download.

Implementation:

function exportToCsv(data, columns, filename) { ... }

Then each export function.

Add component that shows Buttons for each report.

Add to InventoryPage: after NotificationSystem maybe.

Add import of ExportManager.

Add <ExportManager />.

Now produce code.

We must ensure we close tags properly.

Let's craft ExportManager.tsx:

It will accept props: items, movements, printerSupplies, maintenances, softwareLicenses maybe.

But we can get data via useSupabaseInventory inside component, or pass as props.

Simpler: ExportManager receives necessary data as props from InventoryPage.

We'll implement component that receives all data and renders buttons.

Now write code.

After that, modify InventoryPage to import ExportManager and render it.

Add import at top.

Add <ExportManager items={items} movements={movements} supplies={printerSupplies} maintenances={maintenances} />.

Make sure to close.

Now produce final answer with ＜dyad-write> tags for each file.

We need to output only one ＜dyad-write> per file. So first write ExportManager.tsx, then modify InventoryPage.tsx.

Let's do it.
</think><dyad-write path="src/components/inventory/ExportManager.tsx" description="Creating an export manager component with buttons to export assets, supplies, supply exits, maintenance, and movements.">
"use client";
